...
import TopUpTransactionDetails from '../../components/topUp/topUpTransactionDetails'
...
export default function TopUpRecords() {
  ...
  useEffect(() => {
    async function fetchTopUpTransactions() {
      try {
        const res = await getTransactions({ type: 'topup' })
        setTransactions(res)
        setFilteredTransactions(res)
      } catch (error) {
        console.error('Failed to fetch top-up transactions:', error)
      }
    }
  ...
  useEffect(() => {
    const filtered = transactions.filter(
      (txn) =>
        ...
        txn.topupMethod?.toLowerCase().includes(searchTerm.toLowerCase())
    )
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white shadow-md p-6 mt-6 rounded-lg relative">
        ...
        {filteredTransactions.length === 0 ? (
          <div className="text-center text-gray-500 py-10">No top-up transactions found.</div>
        ) : (
          <>
              ...
              <table className="min-w-full text-sm">
                <thead className="text-left bg-gray-100 text-[#0A2A54]">
                  <tr>
                    ...
                    <th className="p-3">Top-Up Method</th>
                  ...
                  {currentItems.map((txn, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-3">{txn.topupMethod || 'N/A'}</td>
