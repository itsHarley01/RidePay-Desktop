...
interface TopUpTransactionDetailsProps {
  transaction: TopUpTransaction;
  onClose: () => void;
}

const TopUpTransactionDetails: React.FC<TopUpTransactionDetailsProps> = ({ transaction, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 bg-opacity-40">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-8 relative">
        ...
        <div className="space-y-4 text-[17px] p-5 bg-gray-100 rounded-md">
          ...
          <div className="flex justify-between">
            <span className="font-medium text-gray-700">Top-Up Method:</span>
            <span>{transaction.topupMethod}</span>
          </div>

          <hr className="my-2 text-gray-300" />

          <div className="flex justify-between">
            <span className="font-medium text-gray-700">Top-Up Amount:</span>
            <span>₱{transaction.topUpAmount.toFixed(2)}</span>
          </div>

          <div className="flex justify-between">
            <span className="font-medium text-gray-700">Top-Up Fee:</span>
            <span>₱{transaction.topUpFee.toFixed(2)}</span>
          </div>
        </div>
